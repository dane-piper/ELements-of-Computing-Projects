...
#  Description: A group of recursive functions that group a list of ints to see if it matches the target value
...
def groupSumClump(start, nums, target):
    if start >= len(nums):
        ...
        ident = 1
        if start < len(nums) - 1:
            next_int = nums[start + 1]
            # This loop adds how many times the same number appears next to one another in the array.
            while nums[start] == next_int:
                ident += 1
                next_int = nums[start + ident]
        return groupSumClump(start + ident, nums, target - nums[start] * ident) or groupSumClump(start + ident, nums, target)
...
# This function adds the number from the array to group1 or group2 and recursively returns if they match
def splitArrayHelper(start, nums, group1, group2):
    if start >= len(nums):
        return group1 == group2
    elif splitArrayHelper(start + 1, nums, group1 + nums[start], group2):
        return True
    elif splitArrayHelper(start + 1, nums, group1, group2 + nums[start]):
        return True
    else:
        return False
...
# This function adds the array number to either group 1 or 2 and recursively returns if it has met the restraints of the problem.
def splitOdd10Helper(start, nums, group1, group2):
    if start >= len(nums):
        return (group1 % 10 == 0 and group2 % 2 == 1) or (group2 % 10 == 0 and group1 % 2 == 1)
    elif splitOdd10Helper(start + 1, nums, group1 + nums[start], group2):
        return True
    elif splitOdd10Helper(start + 1, nums, group1, group2 + nums[start]):
        return True
    else:
        return False
...
# This function adds the array number to group 1 or 2 except when it's a multiple of 5 it adds to group 1 and
# if it's a multiple of 3 it adds to group 2 and recursively checks if it has met the target of the problem.
def split53Helper(start, nums, group1, group2):
    ...
    elif split53Helper(start + 1, nums, group1 + nums[start], group2):
        return True
    elif split53Helper(start + 1, nums, group1, group2 + nums[start]):
        return True
    else:
        return False
