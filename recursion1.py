...
#  Description: Many functions written recursively
...
def countHi2(str):
    if len(str) < 3:
        ...
        if str[-1] == 'i' and str[-2] == 'h' and str[-3] != 'x':
...
def nestParen(str):
    if len(str) == 0:
        ...
        if str[0] == '(' and str[1] == '(' and str[-1] == ')' and str[-2] == ')':
            return True
        else:
            return False
    else:
...
def strDist(str, sub):
    ...
    if len(str) < len(sub):
        ...
        if str == sub:
            ...
            return 0
    elif str[:length1] == sub and str[:-length1] == sub:
        return len(str) - 1
    else:
        if str[:length1] == sub:
            return strDist(str[:-length1], sub)
        else:
